...
public class GameManager {
    ...
    private void updateScoreboard(Player player, PlayerSession session, String itemName, String timeStr) {
        ...
        if (obj == null) {
            ...
            setupTeam(board, "target_val", "§e", 4);
            setupTeam(board, "time_val",
                    Main.getInstance().getMessageManager().getLegacyString("scoreboard.time", "{time}", ""), 2);
            setupTeam(board, "pb_val",
                    Main.getInstance().getMessageManager().getLegacyString("scoreboard.best", "{time}", ""), 1);
        ...
        board.getTeam("target_val").suffix(Component.text(itemName));
        board.getTeam("time_val").suffix(Component.text(timeStr));
        ...
        board.getTeam("pb_val").suffix(Component.text(pbStr));
    }

    private void setupTeam(Scoreboard board, String name, String prefix, int score) {
        Team team = board.registerNewTeam(name);
        String entry = "§" + (char) ('a' + score);
        team.addEntry(entry);
        team.prefix(Component.text(prefix));
        board.getObjective("rsrun").getScore(entry).setScore(score);
    }
    ...
    private void actualStartGame(Player player, Material item) {
        ...
        prepFuture.thenAccept(success -> {
            if (!success) {
                Bukkit.getScheduler().runTask(Main.getInstance(), () -> player
                        .sendMessage(Main.getInstance().getMessageManager().getComponent("messages.world-init-error")));
                return;
            }
            ...
            Main.getInstance().getDatabaseManager().getBestTime(player.getUniqueId(), item).thenAccept(pb -> {
                Bukkit.getScheduler().runTask(Main.getInstance(), () -> {
                    ...
                    WorldManager.getRandomLocation(world).thenAccept(gameSpawn -> {
                        Bukkit.getScheduler().runTask(Main.getInstance(), () -> {
                            if (!player.isOnline())
                                return;
                            ...
                            player.sendMessage(Main.getInstance().getMessageManager()
                                    .getComponent("messages.target-item", "{item}", item.name()));
                            player.sendMessage(
                                    Main.getInstance().getMessageManager().getComponent("messages.game-started"));
                        });
                    });
                });
            });
    ...
    public void finish(Player player) {
        ...
        Main.getInstance().getDatabaseManager().saveRecord(player.getUniqueId(), player.getName(),
                session.getTargetItem(), time);
    ...
    public void openLeaderboardGUI(Player player, Material item) {
        Main.getInstance().getDatabaseManager().getTopRecords(item).thenAccept(records -> {
            Main.getInstance().getDatabaseManager().getBestTime(player.getUniqueId(), item).thenAccept(pb -> {
                Bukkit.getScheduler().runTask(Main.getInstance(), () -> {
                    String title = Main.getInstance().getMessageManager().getLegacyString("gui.leaderboard-title",
                            "{item}", item.name());
                    Inventory gui = Bukkit.createInventory(null, 36,
                            LegacyComponentSerializer.legacySection().deserialize(title));

                    ItemStack header = new ItemStack(item);
                    ItemMeta headerMeta = header.getItemMeta();
                    if (headerMeta != null) {
                        headerMeta.displayName(Main.getInstance().getMessageManager()
                                .getComponent("gui.leaderboard-header", "{item}", item.name()));
                    ...
                    for (int i = 0; i < records.size(); i++) {
                        DatabaseManager.RecordData data = records.get(i);
                        ItemStack rankItem = new ItemStack(Material.PAPER);
                        ItemMeta meta = rankItem.getItemMeta();
                        if (meta != null) {
                            meta.displayName(Main.getInstance().getMessageManager().getComponent("gui.leaderboard-rank",
                                    "{rank}", String.valueOf(i + 1), "{player}", data.playerName()));
                            List<Component> lore = new ArrayList<>();
                            lore.add(Main.getInstance().getMessageManager().getComponent("gui.leaderboard-time",
                                    "{time}", TimeUtil.format(data.time())));
                            meta.lore(lore);
                    ...
                    // Hiển thị kỷ lục cá nhân của người đang xem
                    ItemStack myRecord = new ItemStack(Material.PLAYER_HEAD);
                    ItemMeta myMeta = myRecord.getItemMeta();
                    if (myMeta != null) {
                        myMeta.displayName(Main.getInstance().getMessageManager().getComponent("gui.my-record-title"));
                        List<Component> myLore = new ArrayList<>();
                        String pbStr = (pb == -1)
                                ? Main.getInstance().getMessageManager().getLegacyString("scoreboard.no-best")
                                : TimeUtil.format(pb);
                        myLore.add(Main.getInstance().getMessageManager().getComponent("gui.my-record-time", "{time}",
                                pbStr));
                        myMeta.lore(myLore);
                        myRecord.setItemMeta(myMeta);
                    }
                    gui.setItem(31, myRecord);
