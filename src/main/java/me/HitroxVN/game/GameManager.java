...
public class GameManager {
    ...
    private void processQueue() {
        ...
        if (activeSessions.size() >= maxConcurrent) return;
        ...
        for (UUID uuid : waitingQueue) {
            ...
            if (p != null) p.sendMessage(Main.getInstance().getMessageManager().getComponent("messages.queue-position", "{pos}", String.valueOf(pos++)));
        }
    }
    ...
    private void updateDisplays() {
        ...
        for (PlayerSession session : activeSessions.values()) {
            if (session.isFinished() || session.getStartTime() == 0) continue;
            ...
            if (displayType.equalsIgnoreCase("ACTIONBAR") || displayType.equalsIgnoreCase("BOTH")) {
                player.sendActionBar(Main.getInstance().getMessageManager().getComponent("actionbar.format", "{item}", itemName, "{time}", timeStr));
            }
    ...
    private void updateScoreboard(Player player, PlayerSession session, String itemName, String timeStr) {
        ...
        String pbStr = session.getPersonalBest() == -1 ? Main.getInstance().getMessageManager().getLegacyString("scoreboard.no-best") : TimeUtil.format(session.getPersonalBest());
    ...
    public void start(Player player) {
        ...
        if (Main.getInstance().getConfig().getBoolean("features.random-gui")) openRandomGUI(player);
        else beginCountdownSequence(player, itemManager.getRandomItem());
    }

    private void openRandomGUI(Player player) {
        String titleStr = Main.getInstance().getMessageManager().getLegacyString("gui.title");
        Inventory gui = Bukkit.createInventory(null, 27, LegacyComponentSerializer.legacySection().deserialize(titleStr));
        ...
        for (int i = 0; i < 27; i++) {
            if (i == 4 || i == 22) gui.setItem(i, pointer);
            else if (i < 9 || i > 17) gui.setItem(i, new ItemStack(Material.GRAY_STAINED_GLASS_PANE));
        }
        ...
        new org.bukkit.scheduler.BukkitRunnable() {
            ...
            public void run() {
                ...
                if (cycle.size() > 9) cycle.remove(9);
                for (int i = 0; i < 9; i++) if (i < cycle.size()) gui.setItem(9 + i, new ItemStack(cycle.get(i)));
                player.playSound(player.getLocation(), Sound.BLOCK_NOTE_BLOCK_HAT, 0.5f, 1.5f);
                if (ticks >= maxTicks) {
                    Material winner = cycle.get(4);
                    player.playSound(player.getLocation(), Sound.ENTITY_PLAYER_LEVELUP, 1f, 1.5f);
                    Bukkit.getScheduler().runTaskLater(Main.getInstance(), () -> { player.closeInventory(); beginCountdownSequence(player, winner); }, 20L);
                    this.cancel();
                }
            }
    ...
    private void beginCountdownSequence(Player player, Material item) {
        int maxConcurrent = Main.getInstance().getConfig().getInt("worlds.max-concurrent", 30);
        if (activeSessions.size() < maxConcurrent) worldPrepFutures.put(player.getUniqueId(), WorldManager.prepareWorldFolder(player.getUniqueId()));

        if (Main.getInstance().getConfig().getBoolean("features.countdown")) startCountdown(player, item);
        else handleStartInput(player, item);
    }
    ...
    private void startCountdown(Player player, Material item) {
        ...
        new org.bukkit.scheduler.BukkitRunnable() {
            ...
            @Override
            public void run() {
                if (!player.isOnline()) { worldPrepFutures.remove(player.getUniqueId()); this.cancel(); return; }
                if (count > 0) {
    ...
    private void handleStartInput(Player player, Material item) {
        ...
        if (activeSessions.size() >= maxConcurrent) {
            ...
            player.sendMessage(Main.getInstance().getMessageManager().getComponent("messages.queue-position", "{pos}", String.valueOf(waitingQueue.size())));
        } else actualStartGame(player, item);
    }
    ...
    private void actualStartGame(Player player, Material item) {
        ...
        if (prepFuture == null) prepFuture = WorldManager.prepareWorldFolder(player.getUniqueId());
        if (!prepFuture.isDone()) player.sendMessage(Main.getInstance().getMessageManager().getComponent("messages.world-finishing-up"));

        prepFuture.thenAccept(success -> {
            if (!success) {
                Bukkit.getScheduler().runTask(Main.getInstance(), () -> player.sendMessage(Main.getInstance().getMessageManager().getComponent("messages.world-init-error")));
                return;
            }

            // TRUY VAN DATABASE ASYNC
            Main.getInstance().getDatabaseManager().getBestTime(player.getUniqueId(), item).thenAccept(pb -> {
                Bukkit.getScheduler().runTask(Main.getInstance(), () -> {
                    if (!player.isOnline()) { WorldManager.deleteWorld(player.getUniqueId()); return; }
    ...
    private void sendTitle(Player player, String mainPath, String subPath, String... replacements) {
        ...
        Title title = Title.title(main, sub, Title.Times.times(Duration.ofMillis(100), Duration.ofMillis(800), Duration.ofMillis(100)));
    ...
    public void finish(Player player) {
        ...
        // LUU VAO DATABASE ASYNC
        Main.getInstance().getDatabaseManager().saveRecord(player.getUniqueId(), player.getName(), session.getTargetItem(), time);
        ...
        if (Main.getInstance().getConfig().getBoolean("features.broadcast-winner")) {
            Bukkit.broadcast(Main.getInstance().getMessageManager().getComponent("broadcast.finished", "{player}", player.getName(), "{item}", session.getTargetItem().name().replace("_", " "), "{time}", timeStr));
        }
    ...
    public void restorePlayer(Player player, PlayerSession session) {
        ...
        player.setExp(session.getSavedExp()); player.setLevel(session.getSavedLevel());
    ...
    public void quitEarly(Player player) {
        ...
        if (session != null) WorldManager.deleteWorld(player.getUniqueId());
    ...
    private void applyVictoryEffects(Player player) {
        ...
        if (config.getBoolean("features.victory-sound")) player.playSound(player.getLocation(), Sound.UI_TOAST_CHALLENGE_COMPLETE, 1f, 1f);
        if (config.getBoolean("features.victory-particles")) player.spawnParticle(Particle.FIREWORK, player.getLocation().add(0, 1, 0), 20, 0.5, 0.5, 0.5, 0.1);
        if (config.getBoolean("features.victory-fireworks")) spawnFirework(player.getLocation());
    }

    private void spawnFirework(Location loc) {
        Firework fw = loc.getWorld().spawn(loc, Firework.class);
        FireworkMeta meta = fw.getFireworkMeta();
        meta.addEffect(FireworkEffect.builder().withColor(Color.RED, Color.GREEN, Color.BLUE, Color.YELLOW).withFade(Color.WHITE).with(FireworkEffect.Type.BALL_LARGE).trail(true).flicker(true).build());
        meta.setPower(1); fw.setFireworkMeta(meta); fw.detonate();
    }
    ...
    public void openEditGUI(Player player, int page) {
        ...
        if (playerDraft == null) {
            ...
            for (int i = 0; i < allItems.size(); i++) playerDraft.computeIfAbsent(i / 45, k -> new ArrayList<>()).add(allItems.get(i));
        ...
        String titleStr = Main.getInstance().getMessageManager().getLegacyString("gui.edit-title", "{page}", String.valueOf(page + 1));
        Inventory gui = Bukkit.createInventory(null, 54, LegacyComponentSerializer.legacySection().deserialize(titleStr));
        for (int i = 0; i < 45; i++) if (i < currentPageItems.size()) gui.setItem(i, new ItemStack(currentPageItems.get(i)));
        for (int i = 45; i < 54; i++) gui.setItem(i, new ItemStack(Material.BLACK_STAINED_GLASS_PANE));
        if (page > 0) {
            ItemStack prev = new ItemStack(Material.ARROW);
            ItemMeta prevMeta = prev.getItemMeta();
            prevMeta.displayName(Main.getInstance().getMessageManager().getComponent("gui.prev-page"));
            prev.setItemMeta(prevMeta); gui.setItem(45, prev);
        }
        ItemStack next = new ItemStack(Material.ARROW);
        ItemMeta nextMeta = next.getItemMeta();
        nextMeta.displayName(Main.getInstance().getMessageManager().getComponent("gui.next-page"));
        next.setItemMeta(nextMeta); gui.setItem(53, next);
        player.openInventory(gui);
    }

    public void saveDraft(Player player, Inventory inv) {
        Map<Integer, List<Material>> playerDraft = editDrafts.get(player.getUniqueId());
        if (playerDraft == null) return;
        int page = editPages.getOrDefault(player.getUniqueId(), 0);
        List<Material> pageItems = new ArrayList<>();
        for (int i = 0; i < 45; i++) {
            ItemStack item = inv.getItem(i);
            if (item != null && item.getType() != Material.AIR) pageItems.add(item.getType());
        }
        if (pageItems.isEmpty() && page > 0) playerDraft.remove(page); else playerDraft.put(page, pageItems);
    }
    ...
    public void finalizeEdit(Player player) {
        ...
        if (playerDraft != null) {
            ...
            for (List<Material> pageList : playerDraft.values()) uniqueItems.addAll(pageList);
    ...
    public int getEditPage(Player player) { return editPages.getOrDefault(player.getUniqueId(), 0); }
    public PlayerSession getSession(Player player) { return activeSessions.get(player.getUniqueId()); }
